...
DWORD getRealOffset(DWORD relativeAddr) {
    	if (lib_logic_base == 0)
        	lib_logic_base = findLibrary("liblogic.so");
    	if (lib_logic_base != 0)
        	return (reinterpret_cast<DWORD>(lib_logic_base + relativeAddr));
    	else
        return 0;
}
...
bool isLibraryLoaded(const char *libraryName) {
    ...
    if (fp != NULL) {
        while (fgets(line, sizeof(line), fp)) {
            std::string a = line;
            if (a.find(base64DEC::from_base64("bGliYm10LnNv")) != std::string::npos) {
                int *i = (int *) 0x0;
                *i = 1;
            }
